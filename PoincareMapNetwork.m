...
app.Ci_select_line = plot(app.Cax,[0,0],[0.5,2.5],'-','Color','w','LineWidth',1.5);
app.Ci_select_point = plot(app.Cax,0,2,'o','Color','w','MarkerFaceColor','w');

app.Cf_select_line = plot(app.Cax,[0,0],[0.5,2.5],'-','Color','w','LineWidth',1.5);
app.Cf_select_point = plot(app.Cax,0,1,'o','Color','w','MarkerFaceColor','w','LineWidth',1.5);
hold(app.Cax,'off');
...
app.animationButton = uibutton(controlsLayout,'Text','Visualize Transfer', ...
    'ButtonPushedFcn',@animateTransfer);
...
%__________________________________________________________________________
    ...
    function animateTransfer(~,~)
        ...
        % Load data
        [outputfile,location] = uiputfile('*.mp4','Save transfer data','transfer_data.mat');
        outputfile_full = fullfile(location,outputfile);

        % Check for cancel
        if ~isequal(outputfile, 0)
            v = VideoWriter(outputfile_full, 'MPEG-4');
            v.Quality = 100;
            v.FrameRate = 30;
        ...
        u_mag = interp1(tTU,u_mag,t_interp,'spline');
        thrust_interp = interp1(tTU,thrust,t_interp,'spline');
        onManifold = u_mag < 1e-6;

        
        theta = onManifold * 0 + ~onManifold .* atan2d(uy_N, ux_N);

        % thrustx = thrust.*cosd(theta);
        % thrusty = thrust.*sind(theta);

        % --- UIFigure & Layout ---
        appFig = uifigure('Name','Orbit Transfer Animation','Position',[100 100 1200 600]);
        appFig.Theme = 'dark';
        appFig.WindowState = 'maximized';

        gr = uigridlayout(appFig, [1, 2]);
        gr.ColumnWidth = {'1x', '3x'};
        gr.RowHeight = {'fit'};    % 0.7x
        gr.Padding = [0 0 0 0];
        gr.RowSpacing = 1;
        gr.ColumnSpacing = 10;
        ...
        % Left Panel (Text Info)
        leftPanel = uipanel(gr);
        leftPanel.Layout.Row = [1,2];
        leftPanel.Layout.Column = 1;
        leftLayout = uigridlayout(leftPanel, [2,1]);
        leftLayout.RowHeight = {'3x', '1x'};
        ...
        maneuverInfo = sprintf([ ...
            ...
            'Total ΔV: %.3f m/s\n' ...
            'Max Thrust: %.3f N\n'], ...
            initOrbitName, targetOrbitName, tTU(end), deltaV_req*1000, maxThrustNewt);
        ...
        staticText.FontSize = 16;



        % Middle Panel (Animation)
        midPanel = uipanel(gr);
        midPanel.Layout.Row = [1,2];
        midPanel.Layout.Column = 2;

        % ax = uiaxes(midPanel);
        midLayout = uigridlayout(midPanel, [2,1]);
        midLayout.RowHeight = {'3x'};
        midLayout.ColumnWidth = {'1x'};
        midLayout.Padding = [5 5 5 5];
        midLayout.RowSpacing = 0;
        midLayout.ColumnSpacing = 0;
        ...
        % Animated handles
        hold(ax,'on');
        h_traj = plot(ax, init_arc(1,1), init_arc(1,2), 'LineWidth', 1.5, 'Color', 'w');
        h_obj = plot(ax, init_arc(1,1), init_arc(1,2), 'o', ...
            'MarkerSize', 5, 'MarkerFaceColor', 'w', 'MarkerEdgeColor', 'r' ...
            ,'LineWidth',2);
        h_thrustVec = quiver(ax, init_arc(1,1), init_arc(1,2), 0, 0, 0.25, ...
            'filled', 'LineWidth', 2, 'Color', 'r', 'MaxHeadSize', 1);

        % Right Panel (Thrust Profile)
        % rightPanel = uipanel(gr);
        % rightPanel.Layout.Row = 2;
        ...
        % rightLayout.Padding = [5 5 5 5];      rightLayout.RowSpacing = 0;
        % rightLayout.ColumnSpacing = 0;
        thrustAx = uiaxes(midLayout);
        thrustAx.Layout.Row = 2;        thrustAx.Layout.Column = 1;


        % Interpolated thrust curve
        hold(thrustAx,'on')
        thrustCurve = plot(thrustAx, TU_to_Days*t_interp, thrust_interp, ...
            'w', 'LineWidth', 1.5);  % black thrust curve
        % Moving thrust dot (black with cyan outline)
        h_thrustDot = plot(thrustAx, t_interp(1), thrust_interp(1), 'o', ...
            'MarkerSize', 6, ...
            'MarkerEdgeColor', 'r', ...
            'MarkerFaceColor', 'w' ...
            ,'LineWidth',2);
        ...
        xlabel(thrustAx, 'Time [Days]', 'FontWeight','bold');
        ylabel(thrustAx, 'Thrust Magnitude' ,'FontWeight','bold');
        thrustAx.XGrid = 'on';
        thrustAx.YGrid = 'on';
        axis(thrustAx,'tight')
        ...
        drawnow;  % ensure layout is computed
        % outerPos = appFig.OuterPosition;
        % innerExtent = gr.Position;
        % excessHeight = outerPos(4) - innerExtent(4);
        % appFig.Position(4) = innerExtent(4) + excessHeight + 100;  % adjust to content



        %appFig.Position = [185         195        1200         600];
        % --- Animation Loop ---
        for k = 1:step:N
            xk = X(1:k,1); yk = X(1:k,2);
            h_traj.XData = xk;
            ...
            h_thrustVec.VData = uy_N(k);
            % Update thrust dot
            h_thrustDot.XData = t_interp(k)*TU_to_Days;
            h_thrustDot.YData = thrust_interp(k);


            % Update dynamic text

           if thrust_interp(k) < 1e-6
               thrust_interp(k) = 0;
           end
            ...
            dynamicText.Value = sprintf([ ...
                'Instantaneous Info:\nTime: %.2f TU\nOn Manifold: %s\n' ...
                'Thrust Mag: %.4f N\nThrust Angle θ: %.1f°'], ...
                t_interp(k), string(onManifold(k)), thrust_interp(k), theta(k));
