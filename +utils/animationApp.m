...
% Animated handles
hold(ax,'on');
h_traj = plot(ax, init_arc(1,1), init_arc(1,2), 'LineWidth', 1.5, 'Color', 'y');
h_obj = plot(ax, init_arc(1,1), init_arc(1,2), 'o', ...
    'MarkerSize', 5, 'MarkerFaceColor', 'w', 'MarkerEdgeColor', 'g' ...
    ,'LineWidth',2);
h_thrustVec = quiver(ax, init_arc(1,1), init_arc(1,2), 0, 0, 10, ...
    'filled', 'LineWidth', 2, 'Color', 'g', 'MaxHeadSize', 1);
...
thrustCurve = plot(thrustAx, t_interp, interp1(tTU, u_mag, t_interp, "spline"), ...
    'y', 'LineWidth', 1.5);  % black thrust curve
