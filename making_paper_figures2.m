...
load('scenarios\31Res(3.059)-L1Lya(3.059)_eY.mat')

U = U(find(U(:,3)~=0),:);
vec_ind = floor(linspace(1,length(U),250));
...
ux_n = ux(vec_ind)/umax;
uy_n = uy(vec_ind)/umax;

f = figure();
hold on
plot(Xi(:,1),Xi(:,2),'LineWidth',2,'Color',blue)
plot(Xf(:,1),Xf(:,2),'LineWidth',2,'Color',purple)
plot(init_arc(:,1),init_arc(:,2),'--','LineWidth',1,'Color','k')
plot(thrust_arc(:,1),thrust_arc(:,2),'LineWidth',2,'Color',orange)
quiver(thrust_arc(vec_ind,1), thrust_arc(vec_ind,2),ux_n,uy_n,1,"filled" ...
    ,'Color','r')
...
plot(td,thrust,'LineWidth',2,'Color',orange)
grid on
xlabel('time $t$ [D]','FontSize',14,'Interpreter','latex')
ylabel('Thrust $\mathcal{T}$ [N] (1000 kg S/C)','FontSize',14,'Interpreter','latex')
title('Thrust Profile','FontSize',16,'Interpreter','latex')
