...
draw_shaded_circle(gca,[1-mu,0],Rm, [0.8,0.8,0.8])
plot(xL1, 0, 'rx')
plot(xL2, 0, 'rx')
plot(xL3, 0, 'rx')
...
% 
[minVal, linearIdx] = min(diffs_Mat(:));      % Get minimum value and linear index
[rowIdx, colIdx] = ind2sub(size(diffs_Mat), linearIdx);  % Convert to row, column indices
...
dummyfcn = @(Z) 0;
Z2 = Z0;
%Z2 = fmincon(cost3,Z0,[],[],[],[],[],[],constraint,fmc_opts);
%%
% mmThr = Thrmin*1000;
% while Tmax-mmThr<0
...
[C,Cd, Cdd] = findCf2D(tau,Si,Sf, TOM);
 ...
 plot(1/2,sqrt(3)/2,'rx','LineWidth',1)
 plot(1/2,-sqrt(3)/2,'rx','LineWidth',1)
 text(0.9,0.15,['$\Delta V = $',num2str(dV_opt*1000), ' m/s'] ...
     ,'Interpreter','latex','FontSize',16,'Color',orange)
 hold (gca,'off')
 xlabel('$x$ [DU]','Interpreter','latex')
 ylabel('$y$ [DU]','Interpreter','latex')
 ...
 f5 = figure();
 plot(t_man,Thr,'LineWidth',2,'Color',orange)
 grid on
