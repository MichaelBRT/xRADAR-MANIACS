...
% global variables
global T_em D n TU_to_Days mu mass scalefactor scalefactor2
scalefactor = 1;
scalefactor2 = 1e5;
...
% slack
 ...
 Tmax = 1;
% excess = max(a - Tmax, 0);
% s = sum(excess.^2) / length(excess)
Z0 = [qx;qy;log(scalefactor*TOM);scalefactor2*s];

% TOM0 = Z0(end);
% Tmin = TOM0 * 0.5;
% Tmax = TOM0 * 3.0;
% 
lb = [-Inf(size(Z0,1)-1,1); 0];
ub = [ Inf(size(Z0,1)-1,1); inf];

is_T_design = 1;
params = struct('Wi',XXi,'Wf',XXf,'ti',tti,'tf',ttf, ...
    'M',M,'n',nffs,'A',A,'Ad',Ad,'Add',Add,'is_T_design',is_T_design ...
    ,'Thr_max',Tmax,'T',TOM, 'rho', 100);
...
for i =1:5
    ...
    constraint = @(Z) thrustConstraint(Z,params);
    disp(i)
    Z2 = fmincon(cost,Z0,[],[],[],[],lb,ub,constraint,fmc_opts);
    shistory(i) = Z2(end);
    params.rho = min(params.rho * factor, rhomax);
    Z0 = Z2;
end
disp(shistory)
% mmThr = Thrmin*1000;
...
function [c,ceq] = thrustConstraint(X,params)
global scalefactor2
    s=X(end)/scalefactor2;
    Z=X(1:end-1);
    T = thrust(Z,params);
    Tmax = params.Thr_max;
    c = T - Tmax - s;
...
function [A dA ddA] = getAmatrix(tau, n)
 ...
 function dV = dVcostfcn(Z,params)
    % T_em = 2.361 * 10^6;
    % D = 3.850*10^5;
    % n = 2*pi/T_em;
    % 
    ...
    % tols = 1e-4;    % Target threshold for s
    % 
    % s_history = [];  % Store slack values for monitoring
    % 
    % for k = 1:10  % Limit iterations to avoid infinite loops
    %     % Compute slack based on current acceleration norm:
    %     s = max(max(a) - params.ThrustMax, 0);
    % 
    %     % Store history for plotting/monitoring if needed
    %     s_history(end+1) = s;
    % 
    %     % Update rho if slack is above tolerance
    %     if s > tols
    %         rho = min(rhoinitial * (factor^k), rhomax);
    %     else
    %         break
    %     end
    % end
